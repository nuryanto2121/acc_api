...
namespace Acc.Api.DataAccess
{
    public class ChatSenderRepo
    {
        ...
        public object SaveHeader(ChatSender Model)
        {
            using (IDbConnection conn = Tools.DBConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    Parameters.Add("p_user_id", Model.user_ids);
        ...
        public object GetAllChat(int HeaderId)
        {
            using (IDbConnection conn = Tools.DBConnection(connectionString))
            {
                object _result = null;
                try
                {
                    ...
                    _result = conn.Query<dynamic>("fss_chat_d_getallchat", Parameters, commandType: CommandType.StoredProcedure).ToList();
                }
                catch (Exception ex)
                {
                    throw (ex);
        ...
        public GetChat GetChat(int HeaderId, string user_id)
        {
            using (IDbConnection conn = Tools.DBConnection(connectionString))
            {
                GetChat _result = null;
                try
                {
                    conn.Open();
                    DynamicParameters Parameters = new DynamicParameters();
                    ...
                    _result = conn.Query<GetChat>("fss_chat_d_getchat", Parameters, commandType: CommandType.StoredProcedure).SingleOrDefault();
                }
                catch (Exception ex)
                {
                    throw (ex);
